    ...
    <div class="card-header border-0 pt-5">
        ...
        @if (auth()->user()->role == 'admin')
            <div class="card-toolbar" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover" title="klik untuk menambah unsur pelayanan">
                <a href="{{ route('period.create') }}" class="btn btn-light-primary">
            ...
            <table id="newsTable" class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $period->period == 1? "Januari - Juni": "Juli - Desember" }}</td>
                        <td>{{ $period->year }}</td>
                        <td>{{ $period->is_active == true? "Aktif": "Tidak Aktif" }}</td>
                        <td class="text-center">
                            <button type="button" class="btn btn-light-info btn-sm" data-bs-toggle="modal" data-bs-target="#modalDetail-{{ $loop->index }}">
                                Detail
                            </button>
                            @if (auth()->user()->role == 'admin')
                                <a href="{{ route('period.edit',['period'=>$period]) }}" class=" btn btn-light-success btn-sm">Ubah</a>
                                <form  method="post" class="d-inline" onsubmit="btnDelete('period',{{ $period->id }})">
                                    @csrf
                                    @method('delete')
                                    <button class="btn btn-light-danger btn-sm" >Hapus</button>
                                </form>
                                @if ($period->is_active == false)
                                    <a href="{{ route('period.activate',['period'=>$period]) }}" class=" btn btn-light-primary btn-sm">Aktifkan</a>
                                @endif
                            @endif
                            @include('admin.period.show')
                        </td>
                    </tr>
