...
<div class="container">
    <div class="card">
        <div class="card-body">
            <div class="row">
                <div class="col-md-12">
                    <form action="{{ route('question.update',['question'=>$question]) }}" enctype="multipart/form-data" method="post">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="mb-10">
                                ...
                                <select class="form-select" name="service_id" data-control="select2" data-placeholder="Pilih unsur pelayanan">
                                    ...
                                    @foreach ($services as $service)
                                        <option value="{{ $service->id }}" {{ $service->id == $question->service_id?'selected':'' }} >{{ $service->name }}</option>
                                    @endforeach
                                </select>
                                @error('service_id')
                        ...
                        <div class="col-md-7">
                            <div class="mb-10">
                                ...
                                <textarea class="form-control @error('content') is-invalid  @enderror" name="content" id="content" rows="1">{{ old('content',$question->content) }}</textarea>
                                @error('content')
                                <div class="invalid-feedback d-block">
                                    {{ $message }}
                                </div>
                        ...
                        @livewire('update-answer-form', ['answers' => $question->answers()->get()->toArray()])
                        <div class="col-md-3">
